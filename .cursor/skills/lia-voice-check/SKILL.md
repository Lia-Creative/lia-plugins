...
python3 "[this skill]/scripts/word-check.py" --self-test   # before trusting a run
...
| `spelling to check by sense` | warn | US in one sense, correct Australian in the other — `license` the verb, `meter` the device, `practice` the noun, `program` the software, `dialog` the box. The script cannot read the sense, so it refuses to block, and `ai-patterns.md` §31 carries the same qualifications so the LLM scan does not "correct" them either |
