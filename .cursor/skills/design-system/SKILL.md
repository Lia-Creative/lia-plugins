...
- **0.1.3 (2026-09-02, LIAB-1206, Dan)** — the H3 case is settled the other way: having seen all four levels rendered both ways, Dan ruled that an H3 on a functional surface is a sub-heading inside the interface and stays Söhne 600. The reference states H1/H2/H4 as a deliberate exclusion with that reason, matching the DS code; the 0.1.2 "DS gap" reading is withdrawn (LIAB-1229 is now the write-down, not a selector change).
- **0.1.2 (2026-09-02, LIAB-1206, Dan)** — provisional H1–H4 reading, superseded by 0.1.3 the same day. `verified-against` re-confirmed against GitHub `main`.
