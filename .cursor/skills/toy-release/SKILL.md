...
**Worked again, because it is the next question:** the same tool is promoted `test → uat` and **nothing in the code changed**. Its version does not move — `0.7.2` stays `0.7.2`. Only the register's stage row changes. There is no smallest-true-bump to take, because nothing got better; a version that moves for a promotion is the habit rule 2 replaces. **This holds for `build → test` and `test → uat` only.** A promotion to production is the exception below, and it sets `1.0.0` whether code changed or not — so *no code changed* never means *no bump* when production is the target.

The fixed points: a **new toy starts at `0.0.1`**, and `0.x` means not yet in production.

**And the one exception to rule 2: the promotion to production sets `1.0.0`.** It is the single time a promotion picks a number, and it does so whether or not any code changed — a tool going to production becomes `1.0.0` from wherever it was, and that move is the one release PR a no-code promotion still raises. **A fix landing alongside the promotion does not take a number of its own first:** `0.6.3` plus a bug fix plus the production gate is `1.0.0`, not `0.6.4` and then `1.0.0`. The three rows describe what a version claims; this one fixed point overrides them, once. After it, the three rows again, unchanged.
