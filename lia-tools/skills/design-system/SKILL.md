...
[`references/lia-design-system.md`](references/lia-design-system.md) is organised by the questions a builder actually asks, in this order: colour · tokens · type · components · icons · themes · fonts and licence · known gaps · Toys DS. Read the section your question is in; read the whole file once per session if the work is a screen.
...
- **Never patch either system in the product repo.** An app-local shim is allowed only when genuinely needed, scoped, and marked for removal.
