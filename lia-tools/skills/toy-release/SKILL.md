...
**Worked again, because it is the next question:** the same tool is promoted `test → uat` and **nothing in the code changed**. Its version does not move — `0.7.2` stays `0.7.2`. Only the register's stage row changes. There is no smallest-true-bump to take, because nothing got better; a version that moves for a promotion is the habit rule 2 replaces. **This holds for every stage move except a `0.x` tool's first promotion to production** — including a move back down a stage, which §Rule zero permits on a founder's gate and which never touches the version. A promotion to production is the exception below, and it sets `1.0.0` whether code changed or not — so *no code changed* never means *no bump* when production is the target.

The fixed points: a **new toy starts at `0.0.1`**, and `0.x` means not yet in production.

**And the one exception to rule 2: a `0.x` tool's first promotion to production sets `1.0.0`.** It is the single time a promotion picks a number, and it does so whether or not any code changed — the tool becomes `1.0.0` from wherever in `0.x` it was, and that move is the one release PR a no-code promotion still raises. **It applies once, on the way out of `0.x`, and never again**: a tool already at `1.x` promoted or re-promoted to production takes whatever the three rows say, or nothing at all if no code changed. Read without that scope the sentence would send a tool at `1.4.2` *backwards* to `1.0.0`, which rule 1 forbids anyway. **A fix landing alongside the promotion does not take a number of its own first:** `0.6.3` plus a bug fix plus the production gate is `1.0.0`, not `0.6.4` and then `1.0.0`. The three rows describe what a version claims; this one fixed point overrides them, once. After it, the three rows again, unchanged.
...
**An epic's version is not a tool's version.** `epic-builder` names a chunk of value `charts 1.0`, then `charts 1.1` — that is a name for a slice of work, and it never sets what the manifest says. A `charts 1.0` epic can land as a patch on a tool sitting at `0.3.4`. Under the old table the two roughly co-moved, because the promotion picked the digit; they do not any more, so read them as separate numbering schemes that happen to share a shape.
...
2. **Only if code changed, bump the toy's manifest `version`** in `lia-toy-box` per §Version bumps. **Its own PR, containing nothing else** — titled `release(<toy>): <version> — promote to <stage>`, body linking the promotion ticket. **Open the PR and stop — Chris merges.** The promotion is paused here until he does. **A promotion carrying no code change bumps nothing and raises no PR** — the stage moves in the register at step 4, and an empty release PR is a promotion dressed up as a change. **The one exception is a `0.x` tool's first promotion to production, which sets `1.0.0` code change or not**, and therefore always raises this PR. A tool already at `1.x` gets no such exception. (Before 2 Sep 2026 the promotion picked the digit every time, so this step always had something to do; only production does now — LIAB-1184.)
