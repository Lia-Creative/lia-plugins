...
**Load `_meta/skills/execution-discipline/SKILL.md` first.** Read `Products/Lia Toys/README.md` before sweeping — the convention there is what you're checking against, and it wins over this skill's summary of it.
...
1. **Shape.** Every toy and the toy box has `00 handover/retro-log.md`. Folder names match the convention exactly (`00 handover` … `05 marketing`, unnumbered `meetings/`). Files sitting at a toy's root that belong inside a stage folder. The shape appears nowhere outside `Products/Lia Toys/` — and no Context/Outputs/Resources folders have crept in.
